...
function ClientList(props){
    ...
    const [maxRowClients, setMaxRowClients] = useState([])
    ...
    useEffect(()=>{
        if(typeof props.maxRows !== "undefined"){
            for(let i = 0; i< filteredClientList.length; i++){
                if(i > props.maxRows){
                    setMaxRowClients(prev => [... prev, filteredClientList[i]])
                }
            }
        }else{
            setMaxRowClients(filteredClientList)
        }
